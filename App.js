...
const App = ({route,navigation}) =>{
  ...
  const [addProfilePicture, { data, loading, error }] = useMutation(ADD_PROFILE_PICTURE);
...
const onPress = async() =>  {
  ...
  if (loading) return <Text style={styles.textColor}>Loading...</Text>
  if (error) {
    console.log(error);
...
return (
  <View style={styles.baseContainer}>
     <TouchableHighlight onLongPress = {onLongPress} onPress = {()=>{navigation.navigate('Status', {uri: status.uri, vis: update})}} underlayColor = "white" >
      <View style={[styles.octagon, {borderColor: vis? "grey": "blue"}]}>
        <ImageBackground style= {styles.image} source={avatar}/>
      </View>
      </TouchableHighlight>

      <TouchableHighlight style={styles.tinyLogo}  onPress={onPress} underlayColor = "white" >
      <Image
